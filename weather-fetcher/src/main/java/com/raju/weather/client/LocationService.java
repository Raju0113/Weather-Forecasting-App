...
public class LocationService {
	...
	private Map<String, GeoLocation> coordinatesCache = new ConcurrentHashMap<>(); // simple cache for storing the
																					// coordinates
...
//				+ "&appid="+ Constants.access_Key;
		...
		ResponseEntity<GeoLocation[]> responseEntity = restClient
				                                      .get()
				                                      .uri(url)
				                                      .retrieve()
		...
		if (responseEntity.getStatusCode().is2xxSuccessful()) {
			...
			if (locations != null && locations.length > 0) {
				...
				throw new WeatherServiceException(ErrorCodeEnum.GEOLOCATION_API_FAILED.getCode(),
						ErrorCodeEnum.CITY_NOT_FOUND.getMessage(), HttpStatus.NOT_FOUND);
			}
		} else {

			throw new WeatherServiceException(ErrorCodeEnum.GEOLOCATION_API_FAILED.getCode(),
					ErrorCodeEnum.GEOLOCATION_API_FAILED.getMessage(), HttpStatus.NOT_FOUND);
		}
	}
}
